...
; CALLING SEQUENCE:
;
; PRO GANDALF_HF, templates, galaxy, noise, velScale, sol, emission_setup, l0_gal, lstep_gal,       $
...
;   INT_DISP: float array with the same number of elements as the number of
;	lines in the emission_setup.
;	allows to input the instrumental dispersion (sigma) in km/s
;       at the emission line wavelengths. This is used to broaden the emission
;	line Gaussian templates and to recover the observed line width
; 	from the intrinsic line width (which is needed to measure the line flux)
;       If not supplied, the default value is an array of 0.
;	If supplied, the reported line widths are intrinsic.
...
;       emission line. If the keyword REDDENING is set, the flux and
;	amplitude have been recovered to give the observed values. 
;	If the keyword INT_DISP has been set, the velocity
;       dispersion is already the intrinsic one, otherwise it will be the
;       observed width of the line.
;   BESTFIT: [double] a named variable to receive a vector containing sum of
...
;   EMISSION_TEMPLATES: [double] a named variable to receive an
...
;       Chi^2/DOF~1. 
;	If a constant noise spectrum is provided, the
...
;
; V1.7 Fixed bug on relative strength of satellites lines.
;
; V1.8 Fixed bug on final call for the evalutaion of the residuals
;      during the error computation. Thanks to Helen Johnston and
;      Andrea Cardullo for pointing this problem out.
; 
; V1.9 20150527, HF
;	INT_DISP is taken as an array instead of a constant. 
;	Line fluxes returned in SOL are observed fluxes instead of
;	de-reddened fluxes
;
; V2.0 20150729, HF
; 	set separate limits for narrow and broad lines
;	[0,800 km/s] and [800, 5000 km/s]
;	added documentation to some subroutines
;	

;------------------------------------------------------------------------------------
FUNCTION BVLSN_Solve_pxf, A, B, degree, $
                          FOR_ERRORS=for_errors, NLINES=nlines
; HF: This is a wrapper for BVLS linear solver
; A - templates array [degree+1 + nSSP + nLines, npix] 
; B - data array [npix]
; degree - degree of additive polynomials
; If /for_errors is set, NLINES must be provided so that it can fix the
; weights of Gauss templates to unity.
compile_opt idl2, hidden

; No need to enforce positivity constraints if fitting one
; single template: use faster SVD solution instead of BVLS.
;
s = size(a) ; total # of templates
...
ENDIF ELSE BEGIN               ; Fitting multiple templates
    ...
    ; then fix the weights for these components, by setting the
    ; boundaries of the last NLINES templates to [1.0, 1.0]
...
;   INT_DISP_PIX: instrumental resolution in sigma and in pixels
; Output:
;   returns an array with multiple Gaussians broadened by both intrinsic
;   kinematica and instrument resolution. 
...
;
; The input pars array should contain only the Gaussian paramenter for the lines
...
; The amplitudes for the main lines (kind = 'l') are set to
; emission_setup.A, which are unity. The amplitudes for the satellite
; lines (kind = 'dXX') are computed from the given flux ratio in
; emission_setup.A and the wavelength ratio.
;
; If we wish to fit also the line amplitudes with MPFIT,
; i.e., /for_errors is set, then the pars array should be [A_Hb, V_Hb, S_Hb, A_OIII5007, ...]
; This happens when we are evaluating the errors in the Gaussian
; parameters around the best solution.
;
; The emission line setup is used to determine the line type,
; wavelength, and the flux ratios. The kinematics stored in it are not
; used.  
...
for i = 0,nlines-1 do begin
    ...
    if not keyword_set(for_errors) then begin
        ...
        ; set to unity, then the NNLS weight assigned to each template
        ...
        gaus[*,i]=create_gaussn(X=findgen(npix),PARS=[ampl_i,pars[2*i:2*i+1]],INT_DISP_PIX=int_disp_pix[i_lines[i]])
    endif else begin
        ; Make Gaussian templates amplitudes specified by the input pars array
        gaus[*,i]=create_gaussn(X=findgen(npix),PARS=[pars[3*i:3*i+2]],INT_DISP_PIX=int_disp_pix[i_lines[i]])
    endelse
endfor
...
if i_slines[0] ne -1 then begin
    ...
    for i = 0,n_slines-1 do begin
	    ...
	    ; HF: F_Ha/F_Hb = A_Ha/A_Hb * sigma_Ha(A)/sigma_Hb(A)
	    ;               = A_Ha/A_Hb * sigma_Ha(km/s)*lam_Ha(A)/c/sigma_Hb(km/s)*lam_Hb(A)/c
	    ;               = A_Ha/A_Hb * Lam_Ha/Lam_Hb
	    ; if we can ignore the difference in instrumental resolution
	    ; at the two wavelengths
            a_sline = emission_setup.a[j]*emission_setup.a[j_mline] *(emission_setup.lambda[j_mline]/emission_setup.lambda[j])
            gaus_sline = create_gaussn(X=findgen(npix), $
                                       PARS=[a_sline,pars[i_mline*2]-offset,pars[i_mline*2+1]], $
                                       INT_DISP_PIX=int_disp_pix[j_mline])
        endif else begin
            ...
            a_sline = emission_setup.a[j]*pars[i_mline*3]           *(emission_setup.lambda[j_mline]/emission_setup.lambda[j])
            gaus_sline = create_gaussn(X=findgen(npix), $
...
; structure
;
; The FUNCTARG to be passed on to MPFIT (then to fitfunc_gas) includes
...
if keyword_set(reddening) then n_pars = n_pars + n_elements(reddening)
...
if not keyword_set(for_errors) then begin
    for i=0,nlines*2 -1, 2 do begin
        ...
        parinfo[i+1].limits = [0,8d2]/velscale  ; HF 20150729: Limits for Sgas for narrow lines 
        ; HF 20150729 commented out the code below
    ...
    for i=0,nlines*3 -1, 3 do begin
        ...
        parinfo[i+2].limits = [0,8d2]/velscale ; HF 20150729: Limits for Sgas of narrow lines 
        ; HF 20150729 commented out the code below
...
endelse
; HF 20150729: update limits for broad lines
ind = where(strpos(emission_setup.name[i_lines],'_br') ge 0,ct)
if ct gt 0 then begin
	if ~keyword_set(for_errors) then $
		parinfo[ind*2+1].limits = [8d2,5d3]/velscale else $
		parinfo[ind*3+2].limits = [8d2,5d3]/velscale 
endif
...
if not keyword_set(log10) then begin
    if not keyword_set(reddening) then begin
        functargs={CSTAR:cstar, GALAXY:galaxy, NOISE:noise, EMISSION_SETUP:emission_setup, $
                   KINSTARS:kinstars, VELSCALE:velscale, DEGREE:degree, MDEGREE:mdegree, $
                   GOODPIXELS:goodpixels, L0_GAL:l0_gal, LSTEP_GAL:lstep_gal, $
                   INT_DISP:int_disp, $
...
FUNCTION FITFUNC_GAS, pars, CSTAR=cstar, GALAXY=galaxy, NOISE=noise, EMISSION_SETUP=emission_setup, $
                      ...
                      INT_DISP=int_disp, LOG10=log10, REDDENING=reddening, L0_TEMPL=l0_templ,       $
                      FOR_ERRORS=for_errors
; HF: This is the deviates function for MPFIT. It does the following:
;
; Given input parameters, which contain emission line kinematics and
; reddening E(B-V), along with the LOSVD-convolved SSP templates, 
; solve linearly the weights of the SSP and Gauss templates that
; minimize the difference between the data and the model w/ BVLS, and
; output the deviates array.
;
; If /for_errors is set, then the input parameters also contain the
; emission line amplitudes, and the BVLS solver fixes the weights of the
; Gauss templates at unity so that BVLS does not solve for the
; amplitudes of the Gauss templates. Note that BVLS still solves the
; weights of the SSP templates. This way, Gauss amplitudes are solved
; non-linearly by MPFIT, along with gas kinematics and reddening.
...
IF KEYWORD_SET(reddening) THEN npars = npars + n_elements(reddening)
...
; Generate Gaussian templates convolved by velocity and sigma given in input pars
...
; Stacking all the templates together:
;   1.- Additive legendre polynomials of order 'degree'
...
s = size(cstar)
IF s[0] EQ 2 THEN ntemp = s[2] ELSE ntemp = 1   ; Number of template spectra
c = dblarr(npix,degree+ntemp+nlines+1,/NOZERO)  ; This array is used for estimating predictions
...
ENDIF ELSE BEGIN
    ...
    ; but also include extra internal reddening for emission lines if requested 
...
; Weight all templates with errors to down-weight low S/N data
a = c	
FOR j=0,degree+ntemp+nlines DO a[*,j] = c[*,j]/noise 

; Select the spectral region to fit and solve the overconditioned system
; linearly w/ BVLS: TEMPLATES # WEIGHTS ~= OBSERVED SPECTRUM, solve WEIGHTS
...
; Output weights for all SSP and Gaussian templates
weights = sol[degree+1:n_elements(sol)-1]
; Output array containing the best matching emission-line templates
; In case we have used the keyword FOR_ERRORS, sol_gas should all be 1
...
if keyword_set(reddening) then sol_final = dblarr(nlines*4+n_elements(reddening))
...
for i=0,nlines-1 do begin
    ...
    endelse
    ; Sigma (as observed! intrinsic + instrumental)
    sigma          = sqrt(sol_final[k+3]^2. + int_disp[i_lines[i]]^2.)
    ; Flux of the Emission lines
    ; Flux = Ampl * Sigma(A) * sqrt(2PI) 
    ;      = Ampl * sqrt(2PI) * Sigma(km/s)/c * Lambda_obs
    ;     ~= Ampl * sqrt(2PI) * Sigma(km/s)/c * Lambda_rf * exp(V/c)
...
if not keyword_set(for_errors) then begin
    if keyword_set(reddening) then sol_final[nlines*4:nlines*4+n_elements(reddening)-1] = $
...
if keyword_set(err) then begin
    ...
    if keyword_set(reddening) then esol_final = dblarr(nlines*4+n_elements(reddening))
    k=0 & h=0
    ...
    if not keyword_set(for_errors) then begin
        ...
        for i=0,nlines-1 do begin
            ...
            if esol_final[k+2] gt 0 and esol_final[k+3] gt 0 then begin
                ...
                sigmas_i  = sqrt(sigmas_i^2. + int_disp[i_lines[i]]^2.)
                for j=0,99 do begin 
    ...
    if not keyword_set(for_errors) then begin
        if keyword_set(reddening) then esol_final[nlines*4:nlines*4+n_elements(reddening)-1] = $
...
IF n_elements(int_disp)  LE 0 THEN int_disp = emission_setup.lambda*0d ELSE int_disp = int_disp
...
if not keyword_set(reddening) then $
...
if keyword_set(reddening) then start_pars[2*nlines:2*nlines+n_elements(reddening)-1] = reddening
...
IF KEYWORD_SET(FOR_ERRORS) THEN BEGIN
    if not keyword_set(quiet) then print,'computing errors...'
    ...
    best_pars_2 = mpfit('FITFUNC_GAS',start_pars, FUNCTARGS=functargs, PARINFO=parinfo, $
                        FTOL=1d-2, NFEV=ncalls, ERRMSG=errmsg, PERROR=errors_2, STATUS=status, /QUIET)
    ; -----------------
    ...
    best_pars = best_pars_2 & bestfit = bestfit_2 
...
; 
; HF 20150527: now also recover the emission-line fluxes as observed
if keyword_set(reddening) then begin
    ...
    ; Finally, attenuate the output flux
    sol[l*4] = sol[l*4]*reddening_attenuation_emission
    ; and corresponding errors on the fluxes
    if keyword_set(for_errors) then esol[l*4] = esol[l*4]*reddening_attenuation_emission
endif
...
if not keyword_set(quiet) then begin
    ...
    if not keyword_set(reddening) then print,'Line','Obs. Flux','Ampl.','V   ','sig','A/N',f='(6A10)'
    if keyword_set(reddening)     then print,'Line','Obs. Flux','Ampl.','V   ','sig','A/N',f='(A10,A14,A7,A9,A9,A12)'
    l = indgen(nlines)
