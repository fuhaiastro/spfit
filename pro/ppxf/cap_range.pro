...
;
; PURPOSE:
;   CAP_RANGE(x1,x2) = x1,x1+1,...,x2. In this case x1, x2 should be integers.
;   CAP_RANGE(x1,x2,n) = x1,x1+dx,...,x2 with N integer the result has length N.
;   The result will have the type of x1, but if three parameters are used
;   the result will be at least of type float.
;
; CALLING SEQUENCE:
;   a = CAP_RANGE(x1,x2,n,/OPEN,/LOG,/ASINH,SOFT=soft)
; 
; KEYWORDS:
...
;
; EXAMPLES:
;   IDL> print, cap_range(0,1,5)
;       0.000000  0.250000  0.500000  0.750000  1.00000
;   IDL> print, cap_range(0,1,5,/OPEN)
;       0.100000  0.300000  0.500000  0.700000  0.90000
;   IDL> print, cap_range(1,1e4,5,/LOG)
...
;   V1.31: Renamed cap_range to avoid potential naming conflicts.
;       MC, Oxford 11 October 2013
;-
;----------------------------------------------------------------------
FUNCTION cap_range, xx1, xx2, n, OPEN=open, LOG=log, ASINH=asinh, SOFT=soft
compile_opt idl2
on_error, 2
...
if keyword_set(asinh) then begin 
    ...
    if keyword_set(open) then $ ; Open interval: excluding the extremes
        ...
        2: v = x1 lt x2 ? x1 + indgen(x2-x1+1, TYPE=t) : x1 - indgen(x1-x2+1, TYPE=t)
